...
class ParserModel(Model):
    ...
    def add_placeholders(self):
        ...
        """
        self.input_placeholder = tf.placeholder(tf.int32, (None, self.config.n_features), 'input_placeholder')
        self.labels_placeholder = tf.placeholder(tf.float32, (None, self.config.n_classes), 'labels_placeholder')
        self.dropout_placeholder = tf.placeholder(tf.float32, (), 'dropout_placeholder')
    ...
    def add_embedding(self):
        ...
        Em = tf.nn.embedding_lookup(E, self.input_placeholder)
        embeddings = tf.reshape(Em, (-1, self.config.n_features * self.config.embed_size))
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable('W', shape=(self.config.n_features * self.config.embed_size, self.config.hidden_size), dtype=tf.float32, initializer=xavier_weight_init())
        b1 = tf.get_variable('b1', shape=(1, self.config.hidden_size), dtype=tf.float32, initializer=tf.zeros_initializer)
        ...
        b2 = tf.get_variable('b2', shape=(1, self.config.n_classes), dtype=tf.float32, initializer=tf.zeros_initializer)
    ...
    def run_epoch(self, sess, parser, train_examples, dev_set):
        ...
        for i, (train_x, train_y) in enumerate(minibatches(train_examples, self.config.batch_size)):
            ...
            prog.update(i + 1, [("train loss", loss)]) # DEPRECATED , force=i + 1 == n_minibatches)
